...
class AffiliateService
{
    ...
    public function register(Merchant $merchant, string $email, string $name, float $commissionRate): Affiliate
    {
        if ($merchant->user->email === $email) {
            throw new AffiliateCreateException('Duplicate merchant');
        }
        ...
        if (Affiliate::where('merchant_id', $merchant->id)->whereHas('user', function ($query) use ($email) {
            ...
            throw new AffiliateCreateException('Duplicate Email  in use by another affiliate by someother merchant');
        }
